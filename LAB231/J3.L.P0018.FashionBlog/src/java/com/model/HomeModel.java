...
//    }
    ...
    public String formatDate(String pattern, String input) throws ParseException {
     ...
     public ArrayList<Home> getOverview(int page, int pageSize) throws Exception {
        int from = page * pageSize - (pageSize - 1);
        int to = page * pageSize;
        ArrayList<Home> homes = new ArrayList<>();
        String query = "select id,title,createDate,like,comment from "
                + "( select id,title,createDate,like,comment, ROW_NUMBER() over (order by createDate) as rownumber from Home) "
                + "as home where home.rownumber >= ? and home.rownumber <= ?";
        Connection conn = null;
        DBContext db = null;
        PreparedStatement ps = null;
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String createDate = formatDate("dd-MM-yyyy", rs.getString(3));
                int like = rs.getInt(4);
                int comment = rs.getInt(5);
                String titleGroup = formatDate("MMMM yyyy", rs.getString(3));
                Home home = new Home(idPost, title,createDate, like, comment, titleGroup);
                homes.add(home);
            }
        } catch (Exception ex) {
            throw ex;
